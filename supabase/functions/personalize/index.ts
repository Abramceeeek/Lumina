...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = generate
      ? `Write an engaging, factual ~${(targetMinutes ?? 5) >= 15 ? 900 : 500}-word article for a ${difficulty}-level ${language} learner about: ${topic}. ${focusInstr} Adjust vocabulary and sentence complexity appropriately. Make it genuinely interesting and self-contained. Then add learning scaffolding drawn from THIS article. Respond with ONLY a JSON object (no markdown fences, no preamble): {"title": string, "body": string[], "vocabulary": [{"word": string, "definition": string}], "quiz": [{"type":"mc","q":string,"opts":[string,string,string,string],"correct":number},{"type":"mc","q":string,"opts":[string,string,string,string],"correct":number},{"type":"open","q":string,"placeholder":string}], "branches": [{"title":string,"description":string}]}. "vocabulary": 4-6 key terms. "quiz": two multiple-choice then one open; "correct" is the 0-based index. "branches": 4-5 related next topics.`
