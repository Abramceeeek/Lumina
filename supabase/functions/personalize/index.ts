...
serve(async (req) => {
  ...
  try {
    ...
    if (!anthropicKey && !groqKey) return json({ error: 'Server AI key not configured' }, 500);
...
${(body ?? []).join('\n\n')}`;
    ...
    const maxTokens = generate ? 2800 : 1500;
    // Claude when the project has a key (best quality); Groq's free tier otherwise.
    const text = anthropicKey ? await callClaude(anthropicKey, prompt, maxTokens) : await callGroq(groqKey as string, prompt, maxTokens);
    const modelLabel = anthropicKey ? 'Claude' : 'Llama (Groq)';
    ...
    if (generate) {
      ...
      return json({
        ...
        note: `Generated by ${modelLabel} (hosted) · ${topic} · ${difficulty}`,
      });
    }

    const paras = paragraphs(text);
    return json({ body: paras.length ? paras : body, note: `Personalized by ${modelLabel} (hosted) · ${language} · ${difficulty}` });
  } catch (e) {
    return json({ error: String(e) }, 400);
  }
});

// Groq speaks the OpenAI chat-completions shape; same default model as the app's
// BYOK catalog (apps/mobile/src/ai/catalog.ts).
